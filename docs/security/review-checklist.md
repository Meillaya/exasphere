...
- [ ] `runner-cleanliness-proof.json` was validated with `qa/runner_cleanliness_proof_check.py` and includes JIT config, clean-machine boot/run identity, or a hashed ephemeral registration receipt; no-reuse evidence; runner removal receipt when applicable; run URL, and links to `protected-environment-review.json` plus `runner-substrate-proof.json`; GitHub runner labels alone were not accepted as proof.
