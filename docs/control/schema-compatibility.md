...
| `zig-scheduler/protected-environment-review/v1` | `schemas/control/protected-environment-review.v1.schema.json` | Protected manual workflow approval proof; not release approval. |
| `zig-scheduler/runner-substrate-proof/v1` | `schemas/control/runner-substrate-proof.v1.schema.json` | Protected runner substrate proof for QEMU/KVM/tuple/BPF evidence; release-ineligible and not a production capacity claim. |
| `zig-scheduler/runner-cleanliness-proof/v1` | `schemas/control/runner-cleanliness-proof.v1.schema.json` | Protected runner cleanliness companion artifact; release-ineligible and not a production capacity claim. |
...
| evidence-manifest/v1 | protected manual VM proof bundle | yes | Validated by `qa/evidence_manifest_check.py`; PASS keeps marker, benchmark artifact hashes, runner-substrate proof, and runner-cleanliness proof, while SKIP/REFUSE/BLOCKED may explicitly mark benchmark provenance not applicable without fabricating records. |
| protected-environment-review/v1 | protected manual VM proof bundle | yes | Validated by `qa/protected_environment_review_check.py`; records the protected environment approval comment and reviewer/run linkage, and is not release approval. |
| runner-substrate-proof/v1 | protected manual VM proof bundle | yes | Validated by `qa/runner_substrate_proof_check.py`; records QEMU/KVM/tuple/BPF substrate facts without runner cleanliness fields. |
| runner-cleanliness-proof/v1 | protected manual VM proof companion artifact | yes | Validated by `qa/runner_cleanliness_proof_check.py`; records JIT/ephemeral/clean-machine evidence, no-reuse proof, removal receipt, and links to protected review plus runner-substrate proof without extending runner-substrate-proof/v1. |
...
- 2026-07-01: `matrix-run/v1` scheduler-state proof rows now freeze `sched_ext` and `ops` as required nested fields, and `evidence-manifest/v1` requires explicit `outcome` plus explicit `benchmark_provenance.status=not_applicable` for SKIP/REFUSE/BLOCKED proof bundles. Existing PASS manifests remain valid when they keep marker proof and benchmark artifact references.
- 2026-07-02: Todo 1 protected-core proof scaffolding added `qa/protected_core_suite_check.py` for suite shape/row-local/ABI-v3 linkage, `runner-cleanliness-proof/v1` as a companion runner-cleanliness artifact, and `qa/evidence_bundle_compare_check.py` for repeat bundle role/row/hash comparison without performance claims. Todo 7 wires runner-cleanliness proof into manual workflow capture, evidence-manifest roles, fixtures, and docs without treating runner labels alone as cleanliness proof.
- 2026-07-02: Todo 5 protected-core runtime normalization added `qa/protected_core_telemetry_check.py` as a companion validator for VM-live PASS rows. The public `runtime-sample/v1` schema keeps its required fields stable and adds an optional `unavailable_fact` shape for mature telemetry objects (`policy_counters`, `sample_loss`, `dsq_depth`, `queue_latency`, `fairness`, `scheduler_counters`) so missing VM facts can be explicit unavailable/unknown rather than fake zeroes. The protected checker tightens row-level proof by requiring otherwise-optional telemetry fields and rejecting fake numeric counters when sched_ext events are unavailable.
- 2026-07-02: Todo 10 synchronized the public schema notes and backend contract pack around protected-core proof bundles. No v1 required fields or enum values changed; runner substrate, runner cleanliness, protected-environment review, evidence-manifest, benchmark-output, and protected-core companion validators remain additive proof surfaces for VM-only lab evidence.
