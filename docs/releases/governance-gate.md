...
Protected manual VM proof provenance additionally requires `evidence-manifest.json` to pass `qa/evidence_manifest_check.py` with `schemas/control/evidence-manifest.v1.schema.json`. The manifest is a hash index, not a release approval: it records SHA-256 hashes, schema roles, audit id, rollback id, VM marker, supported tuple, BPF metadata or SKIP JSON, daemon events, matrix manifest, benchmark provenance, rollback proof, cleanup proof, host refusal proof, privacy scan, and attestation status while preserving `host_mutation=false`, `release_eligible=false`, and `production_capacity_claim=false`.
