...
std::string_view event_kind_name(EventKind k) {
    ...
    case EventKind::LabRunActive:
        return "lab_run_active";
    case EventKind::StageFinished:
        return "stage_finished";
    case EventKind::JournalRecord:
        return "journal_record";
    case EventKind::MicrovmBoot:
        return "microvm_boot";
    case EventKind::VmMarker:
        return "vm_marker";
    case EventKind::BpfRegister:
        return "bpf_register";
    case EventKind::RuntimeSample:
        return "runtime_sample";
    case EventKind::Rollback:
        return "rollback";
    case EventKind::RollbackCompleted:
        return "rollback_completed";
    case EventKind::Cleanup:
        return "cleanup";
    case EventKind::Validation:
        return "validation";
...
std::optional<EventKind> event_kind_from_name(std::string_view name) {
    if (name == "boot")
        return EventKind::Boot;
    if (name == "marker")
        return EventKind::Marker;
    if (name == "verifier")
        return EventKind::Verifier;
    if (name == "attach")
        return EventKind::Attach;
    if (name == "state_changed")
        return EventKind::StateChanged;
    if (name == "stage_started")
        return EventKind::StageStarted;
    if (name == "lab_run_active")
        return EventKind::LabRunActive;
    if (name == "stage_finished")
        return EventKind::StageFinished;
    if (name == "journal_record")
        return EventKind::JournalRecord;
    if (name == "microvm_boot")
        return EventKind::MicrovmBoot;
    if (name == "vm_marker")
        return EventKind::VmMarker;
    if (name == "bpf_register")
        return EventKind::BpfRegister;
    if (name == "runtime_sample")
        return EventKind::RuntimeSample;
    if (name == "rollback")
        return EventKind::Rollback;
    if (name == "rollback_completed")
        return EventKind::RollbackCompleted;
    if (name == "cleanup")
        return EventKind::Cleanup;
    if (name == "validation")
...
std::string_view action_kind_name(ActionKind k) {
    ...
    case ActionKind::RollbackLabRun:
        return "rollback_lab_run";
    case ActionKind::IncidentDrill:
        return "incident_drill";
    }
    return "unknown";
}
...
std::optional<ActionKind> action_kind_from_name(std::string_view name) {
    if (name == "preflight")
        return ActionKind::Preflight;
    if (name == "run_lab_host_safe")
        return ActionKind::RunLabHostSafe;
    if (name == "run_lab_vm")
        return ActionKind::RunLabVm;
    if (name == "run_lab_microvm_live")
        return ActionKind::RunLabMicrovmLive;
    if (name == "verifier_only")
        return ActionKind::VerifierOnly;
    if (name == "partial_attach")
        return ActionKind::PartialAttach;
    if (name == "observe")
        return ActionKind::Observe;
    if (name == "stop")
        return ActionKind::Stop;
    if (name == "rollback")
        return ActionKind::Rollback;
    if (name == "stop_lab_run")
        return ActionKind::StopLabRun;
    if (name == "rollback_lab_run")
        return ActionKind::RollbackLabRun;
    if (name == "incident_drill")
        return ActionKind::IncidentDrill;
    return std::nullopt;
}
...
json::Value DaemonEvent::to_json() const {
    ...
    if (!run_id.empty())
        v.set("run_id", json::Value(run_id));
    if (!target_id.empty())
        v.set("target_id", json::Value(target_id));
    if (!audit_id.empty())
        v.set("audit_id", json::Value(audit_id));
    if (!rollback_id.empty())
        v.set("rollback_id", json::Value(rollback_id));
    if (!reason.empty())
...
bool event_from_json(const json::Value& v, DaemonEvent& out) {
    ...
    if (!schema || schema->as_string() != kEventSchema)
        return false;
    ...
    if (aid)
        out.action_id = aid->as_string();
    auto* st = v.find("status");
    if (st)
        out.status = st->as_string();
    auto* hm = v.find("host_mutation");
    if (hm && hm->as_bool())
        return false; // must be false
    auto* sq = v.find("seq");
    if (sq)
        out.seq = static_cast<std::uint64_t>(sq->as_int());
    auto* rid = v.find("run_id");
    if (rid)
        out.run_id = rid->as_string();
    auto* tid = v.find("target_id");
    if (tid)
        out.target_id = tid->as_string();
    auto* aud = v.find("audit_id");
    if (aud)
        out.audit_id = aud->as_string();
    auto* rb = v.find("rollback_id");
    if (rb)
        out.rollback_id = rb->as_string();
    auto* rsn = v.find("reason");
    if (rsn)
        out.reason = rsn->as_string();
    return true;
}
...
json::Value OperatorAction::to_json() const {
    ...
    if (!target_id.empty())
        v.set("target_id", json::Value(target_id));
    if (!target_cgroup.empty())
        v.set("target_cgroup", json::Value(target_cgroup));
    if (!audit_id.empty())
        v.set("audit_id", json::Value(audit_id));
    if (!rollback_id.empty())
        v.set("rollback_id", json::Value(rollback_id));
    if (!target_action_id.empty())
        v.set("target_action_id", json::Value(target_action_id));
    return v;
}
...
bool action_from_json(const json::Value& v, OperatorAction& out) {
    if (!v.is_object())
        return false;
    auto* schema = v.find("schema");
    if (!schema || schema->as_string() != kActionSchema)
        return false;
    ...
    if (tc)
        out.target_cgroup = tc->as_string();
    auto* aud = v.find("audit_id");
    if (aud)
        out.audit_id = aud->as_string();
    auto* rb = v.find("rollback_id");
    ...
    if (ta)
        out.target_action_id = ta->as_string();
    return true;
}
