...
struct Parser {
            ...
            if (c == '\\') {
                ...
                switch (esc) {
                    ...
                    case 'u': {
                        ...
                        std::string hex;
                        for (int i = 0; i < 4 && !at_end(); ++i) hex += next();
    ...
    xsprof::json::Value parse_string_value() {
        return xsprof::json::Value(parse_string());
    }
    ...
    xsprof::json::Value parse_number() {
        ...
        if (peek() == '.') { is_double = true; next(); while (!at_end() && peek() >= '0' && peek() <= '9') next(); }
        if (peek() == 'e' || peek() == 'E') {
            is_double = true;
            next();
            if (peek() == '+' || peek() == '-') next();
        ...
        if (is_double) return xsprof::json::Value(std::stod(num_str));
        return xsprof::json::Value(static_cast<long long>(std::stoll(num_str)));
    }

    xsprof::json::Value parse_bool() {
        if (s.substr(pos, 4) == "true") { pos += 4; return xsprof::json::Value(true); }
        if (s.substr(pos, 5) == "false") { pos += 5; return xsprof::json::Value(false); }
        return {};
    }

    xsprof::json::Value parse_null() {
        if (s.substr(pos, 4) == "null") { pos += 4; return xsprof::json::Value(nullptr); }
        return {};
    }
};
...
Value parse(std::string_view input) {
    ...
    p.skip_ws();
    if (!p.at_end()) return {}; // trailing garbage
    return v;
}
