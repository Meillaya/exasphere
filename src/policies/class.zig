    ...
    return struct {
        ...
        pub fn selectNextCore(self: @This(), ready_queue: *std.ArrayList(usize), runtimes: []const RuntimeTask) ?usize {
            return switch (self.policy) {
                .fcfs => fcfs.selectNext(ready_queue),
                .round_robin => round_robin.selectNext(ready_queue),
                .cfs_like => if (chooseBestReadyTask(RuntimeTask, runtimes, ready_queue.items)) |choice|
        ...
        pub fn shouldPreemptCore(
            ...
            return switch (self.policy) {
                ...
                .cfs_like => blk: {
                    const choice = chooseBestReadyTask(RuntimeTask, runtimes, ready_queue) orelse break :blk false;
        ...
        pub fn keepsRunningSelection(self: @This(), task: RuntimeTask) bool {
            return self.policy == .cfs_like and task.state == .running;
        }
...
fn chooseBestReadyTask(comptime RuntimeTask: type, runtimes: []const RuntimeTask, ready_queue: []const usize) ?ReadyChoice {
