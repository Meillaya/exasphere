...
pub fn main(init: std.process.Init) !void {
    ...
    try runForeground(allocator, options.state_dir);
}

fn runForeground(allocator: std.mem.Allocator, state_dir: []const u8) !void {
    ...
    while (try stdin_reader.interface.takeDelimiter('\n')) |line| {
        ...
        try appendAction(allocator, &output, line, seq);
        seq += 1;
    }
...
fn appendAction(allocator: std.mem.Allocator, output: *std.ArrayList(u8), line: []const u8, seq: usize) !void {
    var event: std.ArrayList(u8) = .empty;
    defer event.deinit(allocator);
    var writer = std.Io.Writer.Allocating.fromArrayList(allocator, &event);
