...
- `2026-06-12T12:39:00-04:00` — **T08 / foreground daemon control surface:** Added `zig-scheduler-daemon` and a `zig build daemon` step with foreground stdin action processing, read-only ready/preflight events, malformed-action refusals, host-mutation refusals, and a bounded event journal under an explicit relative state directory. This remains disabled-safe control plumbing only; it does not bind a network port, start QEMU, load BPF, or mutate scheduler state.
